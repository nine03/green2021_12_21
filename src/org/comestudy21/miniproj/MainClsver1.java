	...
	private static void input() {
		...
		while (true) {
			...
			for (int i = 0; i < top; i++) {
				if (name.equals(sArr[i].name)) {
					flag=true;
					out.println("같은 이름이 없습니다. 계속입력해주세요.");
					break;
				}
			}
			
			if(flag==false) {
				out.println("같은 이름 입니다. 다시 입력해주세요");
				out.print("이름>> ");
				name = scan.next();
				break;
			}
		...
		sArr[top++] = new Student(sequence++, name, newKor, newEng, newMat);
		out.printf("%s학생의 정보가 입력되었습니다. \n", name);
	}
